...
function ssip(fichier_map::String, fichier_consigne_amrs::String)
    ...
    for c in consigne_amrs
        amr::Vector{Config}, cout_trajet = algo_AStar_Time(map, c.dep, c.arr, c.date_depart, matrice_surete, amrs)
        push!(amrs, amr)
        push!(couts_trajets, cout_trajet)
        if amr[1].interv != (0,0)
    ...
    affiche_final(consigne_amrs, amrs, couts_trajets)
end