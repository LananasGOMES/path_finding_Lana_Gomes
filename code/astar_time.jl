...
function algo_AStar_Time(map::Matrix{Char}, dep::Tuple{Int64,Int64}, arr::Tuple{Int64,Int64}, date_debut::Int64, matrice_surete::Matrix{Suite_Inter}, amrs::Vector{Vector{Config}})
    cameFrom, config = parcours_AStar_Time(map,dep,arr, date_debut, matrice_surete, amrs) 
    # Si on a trouvé un chemin
    if config.interv != (0,0)
        return chemin_AStar_Time(map, dep, config, cameFrom)
    end
    # Sinon on retourne l'erreur observé
    return [config], -1
end

function parcours_AStar_Time(map::Matrix{Char}, dep::Tuple{Int64,Int64}, arr::Tuple{Int64,Int64}, date_debut::Int64, matrice_surete::Matrix{Suite_Inter}, amrs::Vector{Vector{Config}})
...
function chemin_AStar_Time(map::Matrix{Char}, dep::Tuple{Int64,Int64}, arr::Config, cameFrom::Dict{Etat, Tuple{Int64,Config}})
    ...
    while current.state != dep
        push!(trajet, current)
        cout_trajet += cout(map, current.state)
        current = (cameFrom[Etat(current.state, current.interv)])[2]
    end
    push!(trajet, current)
    return reverse(trajet), cout_trajet
end