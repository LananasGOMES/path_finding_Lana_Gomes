    ...
    for amr in amrs
        for i in 1:size(amr,1)
            ...
            if amr[i].t == current.interv[2] + 1
                if i != 1
                    return filter(x->x.state!=amr[i-1].state, positions)
...
function successors_time(map::Matrix{Char}, current::Config, matrice_surete::Matrix{Suite_Inter}, amrs::Vector{Vector{Config}})
    ...
    successeurs_spatial::Vector{Tuple{Int64,Int64}} = successeur(map, current.state)
    danger_echange = false
    end_t = 0
    # Pour chaque voisins spaciallement accessible de current
    for s in successeurs_spatial
        # On délimite un intervalle pendant lequel le voisin peut être accèdé
        start_t = current.t + cout(map,s)
        if current.interv[2] != -1 && start_t <= current.interv[2] + 1
            end_t = max(current.t + cout(map,s), current.interv[2])
        elseif current.interv[2] == -1
            end_t = -1
        end
        # Si (start_t,end_t) est bien un intervalle valide
        if (end_t >= start_t) || (end_t == -1)
            # On regarde chaque intervalle sécurisé de la case
            for safe in matrice_surete[s[1],s[2]]
                # Si la tâche est réalisable dans safe
                if !((safe[1] > end_t && end_t >= 1) || (safe[2] < start_t && safe[2] != -1))
    ...
    # S'il y a un échange de positions possible on retire le successeurs en question
    if danger_echange
...
function affiche_final(consigne_amrs::Vector{Consigne_AMR}, amrs::Vector{Vector{Config}}, couts_trajets::Vector{Int64})
    ...
    for i in 1:size(amrs,1)
        ...
        if amrs[i][1].interv != (0,0)
            print(" (symbole = $(Char(96 + i))) : $(size(amrs[i],1)) pas,")
            print(" coût=$(couts_trajets[i]),")
            println(" mission : t=$(consigne_amrs[i].date_depart)->t=$(amrs[i][size(amrs[i],1)].t)")
            if tous_reussi
                max_temps = max(max_temps, amrs[i][size(amrs[i],1)].t)
            end
        elseif amrs[i][1].t == 0
            print(" : La case de départ de la mission $(consigne_amrs[i].nom) est occupé ")
            println("départ=$(consigne_amrs[i].dep), t=$(consigne_amrs[i].date_depart))")
            tous_reussi = false
        else
